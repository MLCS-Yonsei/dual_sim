...
namespace odometry 
{
  ...
  getOdometry::getOdometry() {}

  getOdometry::~getOdometry() {}

  // Load the controller
  void getOdometry::Load(physics::ModelPtr parent,
      sdf::ElementPtr sdf) 
  {
    ...
    ros::SubscribeOptions so =
      ros::SubscribeOptions::create<geometry_msgs::Twist>(command_topic_, 1,
          boost::bind(&getOdometry::cmdVelCallback, this, _1),
          ros::VoidPtr(), &queue_);
    ...
    callback_queue_thread_ = 
      boost::thread(boost::bind(&getOdometry::QueueThread, this));

    // listen to the update event (broadcast every simulation iteration)
    update_connection_ = 
      event::Events::ConnectWorldUpdateBegin(
          boost::bind(&getOdometry::UpdateChild, this));

  }

  // Update the controller
  void getOdometry::UpdateChild()
  {
  ...
  void getOdometry::cmdVelCallback(
  ...
  void getOdometry::publishOdometry(double step_time)
  {
  ...
  tf::Transform getOdometry::getTransformForMotion(double linear_vel_x, double linear_vel_y, double angular_vel, double timeSeconds) const
  {
    tf::Transform tmp;
    tmp.setIdentity();
  ...
  GZ_REGISTER_MODEL_PLUGIN(getOdometry)
}