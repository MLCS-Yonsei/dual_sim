...
class ActorNet:
    ...
    def __init__(self,num_states,num_actions):
        ...
        with self.g.as_default():
            ...
            self.Conv1_a, self.Conv2_a, self.Conv3_a, self.W1_a, self.W1p_a, self.B1_a, self.W2_a, self.B2_a, self.W3_a, self.B3_a,\
            self.actor_state_in, self.actor_model = self.create_actor_net(num_states, num_actions)
            

                                   
            #target actor network model parameters:
            self.t_Conv1_a, self.t_Conv2_a, self.t_Conv3_a, self.t_W1_a, self.t_W1p_a, self.t_B1_a, self.t_W2_a, self.t_B2_a, self.t_W3_a, self.t_B3_a,\
            ...
            self.actor_parameters = [self.Conv1_a, self.Conv2_a, self.Conv3_a, self.W1_a, self.B1_a, self.W2_a, self.B2_a, self.W3_a, self.B3_a]
            ...
            self.sess.run([
                ...
                self.t_Conv3_a.assign(self.Conv3_a),
            ...
            self.update_target_actor_op = [
                ...
                self.t_Conv3_a.assign(TAU*self.Conv3_a+(1-TAU)*self.t_Conv3_a),
    ...
    def create_actor_net(self, num_states=36*4 + 2 + 2, num_actions=2):
        """ Network that takes states and return action """
        N_HIDDEN_1 = 200
        N_HIDDEN_2 = 200
        actor_state_in = tf.placeholder("float",[None, num_states])
        
        # DQN parameters (input: 64*64*4)
        # layer#  1   2   3
        # filter  32  64  64
        # stride  4   2   1
        # window  8*8 4*4 3*3

        # Our parameters (input: 36*4)
        # layer#  1   2   3
        # filter  16  32  32
        # stride  2   2   1
        # window  4   4   2

        Conv1_a = tf.Variable(tf.random_uniform([4, 4, 16], -1/math.sqrt(16*4), 1/math.sqrt(16*4)))
        Conv2_a = tf.Variable(tf.random_uniform([4, 16, 32], -1/math.sqrt(32*4), 1/math.sqrt(32*4)))
        Conv3_a = tf.Variable(tf.random_uniform([2, 32, 32], -1/math.sqrt(32*2), 1/math.sqrt(32*2)))
        W1_a = tf.Variable(tf.random_uniform([9*32,N_HIDDEN_1],-1/math.sqrt(9*32),1/math.sqrt(9*32)))
        W1p_a = tf.Variable(tf.random_uniform([4,N_HIDDEN_1],-1/math.sqrt(4),1/math.sqrt(4)))
        B1_a = tf.Variable(tf.random_uniform([N_HIDDEN_1],-0.0003,0.0003))
        W2_a = tf.Variable(tf.random_uniform([N_HIDDEN_1,N_HIDDEN_2],-1/math.sqrt(N_HIDDEN_1),1/math.sqrt(N_HIDDEN_1)))
        B2_a = tf.Variable(tf.random_uniform([N_HIDDEN_2],-0.0003,0.0003))
        W3_a = tf.Variable(tf.random_uniform([N_HIDDEN_2,num_actions],-0.0003,0.0003))
        B3_a = tf.Variable(tf.random_uniform([num_actions],-0.0003,0.0003))

        lidar_state_in = actor_state_in[:,:36*4]
        lidar_state_in = tf.reshape(lidar_state_in, [tf.shape(lidar_state_in)[0], 36, 4])
        prev_action_target = actor_state_in[:,36*4:]
        C1_a = tf.nn.conv1d(lidar_state_in, Conv1_a, 2, padding='SAME')
        C1_a = tf.nn.softplus(C1_a)
        C2_a = tf.nn.conv1d(C1_a, Conv2_a, 2, padding='SAME')
        C2_a = tf.nn.softplus(C2_a)
        C3_a = tf.nn.conv1d(C2_a, Conv3_a, 1, padding='SAME')
        C3_a = tf.nn.softplus(C3_a)
        C3_a = tf.layers.flatten(C3_a)

        H1_a = tf.nn.softplus(tf.matmul(C3_a,W1_a)+tf.matmul(prev_action_target,W1p_a)+B1_a)
        H2_a = tf.nn.softplus(tf.matmul(H1_a,W2_a)+B2_a)
        actor_model = tf.nn.tanh(tf.matmul(H2_a,W3_a) + B3_a)

        return Conv1_a, Conv2_a, Conv3_a, W1_a, W1p_a, B1_a, W2_a, B2_a, W3_a, B3_a, actor_state_in, actor_model
